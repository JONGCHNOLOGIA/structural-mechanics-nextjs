...
export function solveBeamFull(L, supports, loads, EIspec, N = 400) {
  ...
  loads.forEach((l) => {
    ...
    // 모멘트류는 M(x)에 +m으로 들어가면 ΣM 식에는 −m으로 들어가야 한다(고정단 반력모멘트의
    // 계수가 −1인 것과 같은 규약). 적용 모멘트는 M(x)에 −M0으로 들어가므로 ΣM에는 +M0이다.
    // 예전에는 여기도 −M0으로 두어서, 모멘트 하중을 얹으면 반력 부호가 뒤집히고
    // 자유단에서 M(L)이 0으로 닫히지 않았다.
  ...
  // 변형에너지 ∫M²/2EI나 가상일 ∫Mm/EI를 구할 때 쓸 구간 중점 표본.
  // 절점 값만으로 적분하면, 모멘트 하중이나 고정단처럼 M(x)가 절점에서 뛰는 자리에서
  // "뛴 뒤의 값"을 그 구간 전체에 쓰게 되어 눈에 띄게 어긋난다. 중점은 뛰는 자리를 비껴간다.
  const midPts = xm.map((x, i) => ({
    x,
    M: momentFromLoads(x) + unknowns.reduce((t, u, j) => t + R[j] * basisAt(u, x), 0),
    EI: EIm[i],
    dx: h,
  }));

  return { determinacy: info.kind, ...info, supports: supportsR, pts, midPts, EI: EIspec, C1, C2 };
}
...
//
// fmtX는 x를 화면 단위로 바꿔 적기 위한 포맷터 — 안 넘기면 SI(m) 숫자 그대로 쓴다.
// 반환: [{ kind, x, text, why }]
export function conditionsFor(L, supports, loads, fmtX) {
  const out = [];
  const label = fmtX || ((v) => String(Number(v.toFixed(6))));
  const f = (v) => label(v);
