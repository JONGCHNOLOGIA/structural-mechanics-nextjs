...
// 딱 떨어지는 값은 소수점을 아예 안 붙인다: "10,000.000"이 아니라 "10,000".
// 소수부가 있을 때만 설정된 자리수(DECIMALS)까지 보여주되, 그 자리수 안에서 뒤에 남는 0은 버린다
// ("3.140"이 아니라 "3.14"). 설정 자리수보다 잘게 쪼개진 값은 예전처럼 반올림되므로,
// 반올림한 결과가 정수가 되면 소수점 없이 나온다.
export function fmt(v) {
  if (!isFinite(v)) return '—';
  // 반올림하면 0이 되는 아주 작은 음수는 "-0"으로 찍힌다 — 부호를 떼서 그냥 0으로 보이게 한다.
  const n = Object.is(v, -0) || Math.abs(v) < Math.pow(10, -DECIMALS) / 2 ? 0 : v;
...
export function fmtSci(v, sig = 3) {
  ...
  // 10⁰은 적어봐야 읽는 데 방해만 된다 ("193.055 × 10⁰"). 지수가 0이면 숫자만 쓴다.
  if (exp === 0) return `${sign}${mantissa.toFixed(sig)}`;
