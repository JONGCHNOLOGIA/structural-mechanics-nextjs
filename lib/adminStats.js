...
export async function fetchVisitCountsBySubtopic() {
  const { data, error } = await supabase.from('topic_visits').select('chapter_num, subtopic_slug');
  if (error || !data) return [];

  const map = new Map();
  for (const row of data) {
    const key = `${row.chapter_num}::${row.subtopic_slug}`;
    map.set(key, { chapterNum: row.chapter_num, slug: row.subtopic_slug, visitors: (map.get(key)?.visitors || 0) + 1 });
  }
