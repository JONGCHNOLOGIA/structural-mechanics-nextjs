...
function genShearForceEquation() {
  ...
  return {
    prompt:
      `A cantilever beam AB (fixed at A, free at B) of length L = ${L} m supports a uniform load of intensity q = ${q} kN/m acting over the full ` +
      `length. The beam has E = ${EGPa} GPa and I = ${IDisp} × 10⁶ mm⁴. By integrating the shear-force equation, determine (a) the angle of ` +
      `rotation θB and (b) the deflection δB at the free end.`,
