...
function twoBlockCrossSection(width, hBottom, hTop, matBottom, matTop) {
  return {
    widthLabel: `${width}mm`,
    blocks: [
      { widthPx: pxScale(width), heightPx: pxScale(hBottom), heightLabel: `${hBottom}mm`, label: matBottom.name.split('(')[0], fill: BLOCK_COLORS[0].fill, stroke: BLOCK_COLORS[0].stroke },
      { widthPx: pxScale(width), heightPx: pxScale(hTop), heightLabel: `${hTop}mm`, label: matTop.name.split('(')[0], fill: BLOCK_COLORS[1].fill, stroke: BLOCK_COLORS[1].stroke },
    ],
  };
}
...
function genCompositeBeams() {
  ...
  return {
    prompt:
      `길이 L=${L}m인 단순보가 전길이에 걸쳐 등분포하중 q=${q}kN/m를 받는다. 이 보는 폭 ${width}mm, 두 재료로 이루어진 복합 단면으로, 아래쪽 블록은 ` +
      `${matBottom.name}(E=${matBottom.GPa}GPa, 높이 ${hBottom}mm)이고 위쪽 블록은 ${matTop.name}(E=${matTop.GPa}GPa, 높이 ${hTop}mm)이다. ` +
      `최대모멘트가 작용하는 단면에서 (1) 하단을 기준으로 한 중립축 위치 ȳ, (2) 최하단 응력, (3) 최상단 응력을 구하시오.`,
    answers: [
      `M(최대) = qL²/8 = ${fmt(moment)} kN·m`,
...
function genTransformedSection() {
  ...
  return {
    prompt:
      `길이 L=${L}m인 단순보가 전길이에 걸쳐 등분포하중 q=${q}kN/m를 받는다. 이 보는 ${matBottom.name}(E₁=${matBottom.GPa}GPa) 위에 ` +
      `${matTop.name}(E₂=${matTop.GPa}GPa)를 덧댄 복합 단면으로, 아래쪽 블록은 폭 ${width}mm·높이 ${hBottom}mm, 위쪽 블록은 폭 ${width}mm·높이 ${hTop}mm이다. ` +
      `최대모멘트가 작용하는 단면에서, 환산계수 n=E₂/E₁을 이용한 환산단면법으로 (1) 환산계수 n, (2) 중립축 위치 ȳ(하단 기준), (3) 최하단(재료 1) 응력을 구하시오.`,
    answers: [`M(최대) = qL²/8 = ${fmt(moment)} kN·m`, `n = ${fmt(n)}`, `ȳ = ${fmt(r.ybar / mmF)} mm (하단 기준)`, `σ(최하단) = ${fmt(sBottom / MPaF)} MPa`],
    diagram: {
      type: 'beamAndCrossSection',
      beamProps: { support: 'simple', spanLabel: `L=${L}m`, loads: [{ kind: 'udl', label: `q=${q}kN/m` }] },
      crossSectionProps: twoBlockCrossSection(width, hBottom, hTop, matBottom, matTop),
...
function genInclinedLoads() {
  ...
  return {
    prompt:
      `길이 L=${L}m인 단순보가 전길이에 걸쳐 등분포하중 q=${q}kN/m를 받는다. 이 보의 폭 b=${width}mm, 높이 h=${height}mm인 직사각형 단면은 ` +
      `수직축(z축)에서 α=${alpha}° 기울어져 있다. 최대모멘트가 작용하는 단면에서 (1) 모멘트의 z축 성분 Mz와 y축 성분 My, ` +
      `(2) 단면 모서리 점(y=h/2, z=b/2)에서의 굽힘응력 σ를 구하시오.`,
    answers: [
      `M(최대) = qL²/8 = ${fmt(moment)} kN·m`,
...
function genShearForceEquation() {
  ...
  return {
    ...
    diagram: { type: 'beam', props: { support: 'cantilever', spanLabel: `L=${L}m`, loads: [{ kind: 'udl', label: `q=${q}kN/m` }] } },
  };
}
