...
export default function MaxStressPointDiagram({ widthLabel, heightLabel, MLabel, VLabel, yFrac }) {
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} style={{ width: '100%', maxWidth: 220, margin: '4px auto 0', display: 'block', fontFamily: "'Noto Serif KR', Georgia, 'Times New Roman', serif" }}>
      ...
      <text x={cx + 10} y={pointY + 4} fontSize="13" fontWeight="800" fill="#C3002F">Point</text>

      <text x={x0 - 10} y={cy + 4} fontSize="12.5" fill="#51626F" textAnchor="end" />
      <text x={cx} y={y0 - 34} fontSize="15" fontWeight="800" fill="#4A5FBF" textAnchor="middle">{MLabel}</text>
      <text x={cx} y={y0 - 16} fontSize="15" fontWeight="800" fill="#1E7F72" textAnchor="middle">{VLabel}</text>

      <text x={cx} y={y0 + rh + 18} fontSize="12.5" fill="#51626F" textAnchor="middle" fontWeight="700">{widthLabel}</text>
      <text x={x0 - 8} y={cy} fontSize="12.5" fill="#51626F" textAnchor="end" transform={`rotate(-90 ${x0 - 8} ${cy})`}>{heightLabel}</text>
    </svg>
  );
}
